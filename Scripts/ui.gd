...
#sprint label
@onready var sprint_label: Label = $HUD/sprint_label
var interior := false
...
func sprint_display():
	if interior == true:
		sprint_label.modulate = Color.DIM_GRAY
	else:
		sprint_label.modulate = Color.WHITE
		
	if Input.is_action_pressed("run") and not interior:
		var color = sprint_label.modulate
		color.a = 1.0
		sprint_label.modulate = color
