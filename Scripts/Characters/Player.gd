...
func _physics_process(delta):
	...
	if not input_dir:
		...
		if Input.is_action_just_pressed("run") and not crouch:
			if is_in_interior:
		...
		if Input.is_action_pressed("run") and not is_in_interior and not crouch and not system_grabe.picked_object:
			...
			else: #si on run pas et qu'on crouch pas on walk
	...
	if Input.is_action_pressed("crouch"):
		...
		collisionshape.position.y = 0.66
		capsule.height = 1.303
	
	elif not ray_cast_crouch.is_colliding(): #si on run pas et qu'on crouch pas on walk
		print("raycast not colliding")
		crouch = false
		var capsule := collisionshape.shape as CapsuleShape3D
		collisionshape.position.y = 0.859
		capsule.height = 1.7
	...
	animator.set("parameters/crouch/blend_position", current_blend_position)
	
	# lerp le blend 2D
	var target_blend_position = Vector2(input_dir.x,0 - input_dir.y)
...
func animate(delta):
	if is_on_floor():#on touche le sole
		animator.set("parameters/ground_air_transition/transition_request", "grounded")
		
		if crouch:
			animator.set("parameters/is_crouching/transition_request", "crouch")#on joue le crouch ou l'idle crouch
			step_interval = 0.8
			footstep_path.pitch_scale = randf_range(speed/5 +1, speed/5 +2)
			
		else:# on est pas crouch
			animator.set("parameters/is_crouching/transition_request", "move")
			#idle standing
			if velocity == Vector3.ZERO:
				animator.set("parameters/iwr_blend/blend_amount", lerp(animator.get("parameters/iwr_blend/blend_amount"), -1.0, delta * ANIMATION_BLEND))
				
			# walk
			if speed >= walk_speed -1 and speed <= walk_speed +1 and not crouch: # si speed est == a walk_speed avec une marge de 1
			...
			# run
			if speed >= run_speed -1 and speed <= run_speed +1 and not crouch: # si speed est == a run_speed avec une marge de 1
				animator.set("parameters/iwr_blend/blend_amount", lerp(animator.get("parameters/iwr_blend/blend_amount"), 1.0, delta * ANIMATION_BLEND))
				step_interval = 0.3
				footstep_path.pitch_scale = randf_range(speed/5 +1, speed/5 +2)
